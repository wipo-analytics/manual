...
In this chapter we look at the use of the [`rplos`](https://github.com/ropensci/rplos) package from [rOpenSci](https://ropensci.org) to access the scientific literature from the [Public Library of Science](https://www.plos.org) using the [PLOS Search API](http://api.plos.org/solr/faq/). The `rplos` package provides a straightforward introduction to the use of APIs in R and is a gentle entry point into a wide range of API packages for the scientific literature.
...
PLOS is important because it provides open access to the full text of peer reviewed research. For researchers interested in working with R in order to access the scientific literature other important packages include [rcrossref](https://docs.ropensci.org/rcrossref/), [openalexR](https://github.com/massimoaria/openalexR) and [bibliometrix](https://www.bibliometrix.org/home/). In particular we recommend the new `openalexR` for accessing over 200 million publication records from [OpenAlex](https://openalex.org/) as the replacement for Microsoft Academic Graph. 

This chapter makes no assumptions about knowledge of R or programming. `rplos` is a good place to start with learning how to access scientific literature in R using Application Programming Interfaces (APIs). Because `rplos` is well organised and the data is very clean it is also a good place to learn some of the basics of working with data in R. This provides a good basis for working with the ROpenSci [fulltext package](https://github.com/ropensci/fulltext). `fulltext` allows you to retrieve scientific literature from multiple data sources and we will deal with that next. 
...
A new object is likely to be a table or list containing data. the sign `<-` gets or passes the results of the function (such as searchplos) to the new object. To specify what we want we first include our data (`yourdata`) and then one or more arguments which control what we get, such as the number of records or the title etc. 
...
```{r plosfields, eval=FALSE, tidy=TRUE, cache=TRUE}
plosfields
```
...
Ok, we now know where we are. Note that a much better option than using `setwd()` is to create a specific project in RStudio with `File > New Project`. This approach has the major advantage of storing all of your files with the same project meaning that they are preserved in the same place when you go back to a project. Another option is to use the [here package](https://github.com/r-lib/here).
...
As we have seen above, PLOS contains 13 journals and in `rplos` the results for a search can be limited to specific journals such as PLOS ONE or PLOS Biology. Note that the short journal names appear to use the old format for PLOS consisting of mixed upper and lowercase characters (e.g. PLoSONE not PLOSONE). A nice easy way to find the short journal names is to use:

```{r journalname, eval=FALSE, cache=TRUE, tidy=TRUE}
journalnamekey()
```

Here we will limit the search to PLOS ONE by adding `fq =` to the arguments and then the `journal_key` argument. Note that the `fq=` argument takes the same options as `fl=`. But, `fq =` filters the results returned by PLOS to only those specified in `fq =`. 

```{r pizza, eval=FALSE, cache=TRUE, tidy=FALSE}
pizza <- searchplos(q = "pizza", fl = c("id", "publication_date", "title", "abstract"), fq = 'journal_key:PLoSONE', start = 0, limit = 20)
head(pizza$data)
```
...
One way to do this is to take our original number of results and then subset in to the data and create a new object containing the value for the number of records in `numFound`. Note that the number of records for a particular query will have gone up by the time that you read this chapter. 

```{r numFoundvalue, eval=FALSE, cache=TRUE, tidy=TRUE}
r <- pizza$meta$numFound
```

To run a new search we can now insert `r` into the limit = value. This will be interpreted as the numeric value of `r` (484 at the time of writing). 

```{r pizznumFound, eval=FALSE, cache=TRUE}
pizza <- searchplos(q = "pizza", fl = c("id", "publication_date", "title", "abstract"), fq = 'journal_key:PLoSONE', start = 0, limit = r)
head(pizza$data)
```
...
r <- searchplos(q = "pizza", fq = 'journal_key:PLoSONE', limit = 0)$meta$numFound
r
```
...
That has returned the full 484 results for PLOS ONE. We could attempt to make life even easier by first getting the results across all PLOS journals. We do this by removing the `fq =` argument limiting the data to PLOS ONE and saving the result in and object we will call `r1`. Note that the number of records will probably have gone up by the time you read this. 
...
This produces 563 results at the time of writing. What happens now if we run our original query using the value of `r1` (563 records) but limiting the results only to PLOS ONE?

```{r crosslimit, eval=FALSE, cache=TRUE, tidy=FALSE}
pizza <- searchplos(q = "pizza", fl = c("id", "publication_date", "title", "abstract"), fq = 'journal_key:PLoSONE', start = 0, limit = r1)
pizza$meta$numFound
```

The answer is that the 484 results in PLOS ONE are returned from the total of 563 across the PLOS journals. Why? The reason this works is that `searchplos()` initially pulls back all of the data from the PLOS API and then applies our entry in `fq =` as a filter. So, in reality the full 563 records are fetched and then filtered down to the 484 from PLOS ONE. In this case, this makes our lives easier because we can use the results across PLOS journals and then restrict the data. 

## Writing the results and using a codebook

We now have a total of 484 results for pizza. We can simply write the results to a .csv file.
...
Note that while synthetic and biology appear inside quotes (suggesting they are a phrase to be searched) in reality the API will treat this as synthetic AND biology. That is, the query will look first for documents that contain the words synthetic AND biology and then for those cases where the words appear within 15 words of each other. In this case we get 2928 results across PLOS (everything) and full texts (`fq = "doc_type:full`) as we can see from this code. 
...
We can narrow the search horizon to ~1 to capture those cases where the terms appear next to each other (within 1 word either to the left or the right) which produces 2120 results at the time of writing.
...
This is actually higher than the total returned on an exact match for the phrase suggesting that there could be cases of "biology synthetic" or other issues (such as punctuation) or API performance that account for the variance. As noted in the `searchplos()` documentation: 
...
This is actually quite a common issue when using both a public interface and an API, the results vary depending on factors on the data provider end that are hidden from our view. As a result, it is a good idea to try different approaches. Even if it is not possible to get to the bottom of any variance it is very useful to note it down in your project codebook to highlight the issue to others who may try and repeat your work. 
...
You should now see a number around 2106 (at the time of writing). Yay! 
...
We can make life simpler by using pipes `%>%` to simplify the code. The advantage of using pipes is that we do not have to keep creating and overwriting temporary objects (see above for `results`). The code is also much easier to read and faster. To learn more about using pipes see this article from [Sean Anderson](http://seananderson.ca/2014/09/13/dplyr-intro.html) or read Hadley Wickham and Garret Grolemund's excellent free online [R for Data Science](https://r4ds.had.co.nz/). Again the query might be a bit slow as the data is fetched back.
...
At the time when this chapter was originally written pipes were a relatively recent innovation in R (see the `magrittr`, `dplyr` and `tidyr` packages). However, pipes make R code faster and much easier to follow and their use is now very common (including the introduction of a base R pipe `|>` that works in a similar but not identical way. While you will need to be familiar with regular R code pipes are very popular because the code is simpler and has a clearer logic (e.g. do this then that).

We now have our data consisting of over 3,598 records (at the time of writing) in a single data frame that we can view.
...
We could also drop columns by position using the following (to remove column 5 and 6). This approach is useful when there are lots of columns to deal with but be aware that things can easily go wrong when selecting by position.

```{r select1, cache=TRUE, eval=FALSE, tidy=TRUE}
test <- select(results, 1:4, 7:9)
```

An easier and better approach is to explicitly drop columns using `-` and keep the others. 
...
```{r unique, eval=FALSE, cache=FALSE}
unique(results$id) # displays unique DOIs as a very long list (base R)
```

```{r distinct, eval=FALSE, cache=FALSE}
n_distinct(results$id) # displays the count of distinct DOIs (dplyr)
```

My personal preferred way of doing this is to use `count()`, because it gives us back a vector of the unique results that we can assign to a vector for later use (rather than a single value). Using `sort = TRUE` we can also identify the duplicated unique identifiers. Using count() in this way is a very powerful time saving trick.
...
This tells us there are 2,251 unique DOIs meaning there were 1,247 duplicates at the time of writing. 
...
### Pivoting data using `pivot_wider()` from `tidyr`

Rather than simply deleting our duplicate DOIs, we could create new columns for each search term and its associated DOI. This will be useful because it will tell us which terms are associated with which records over time. This is easy to do with `pivot_wider()` by providing a column where the names of new columns will be taken from the value that will fill these columns in the arguments. In this case, we want to use `.id` containing our search_terms as the `names_from` (to become new column names) to spread across the table and the DOIs in the `id` column as the `value_from` for the rows. This will then identify the search terms. Note that a warning will be generated in cases where there are duplicated ids. On this case we are happy with that and will ignore the warning.  However, in other cases you will want to address the duplication using the approach suggested in the warning message. As such, expect to experiment. 

```{r pivot_wider, cache=TRUE, eval=FALSE}
wide_results <- results %>% 
  pivot_wider(names_from = .id, values_from = id)
```

This creates a column for each search term with the relevant DOIs as the values. Note that the default is to drop the original column (in this case `.id`) when creating the new columns. 

We now have a data.frame with 2,251 rows that corresponds with the unique ids and the search terms that appear in those documents. This is what we want. If we inspected the results on the terms we would detect a potentially interesting pattern where some documents are only using terms such as synthetic genome or synthetic genomics while others are using only synthetic biology or a mix of terms. 

We have now reduced our data to unique records while preserving our search terms as reference points. The limitation of this approach is that by spreading the DOIs across 4 columns we no longer have a tidy single column of DOIs. Note that you can use the `pivot_longer()` function to convert the data back to long form as needed. 

### Deleting Duplicates

As an alternative, or complement, to pivoting we can use a logical TRUE/FALSE test to filter our dataset. There are a number of functions that perform logical tests in R (see also `which()`, `%in%`, `within()`). In this case the most appropriate choice is probably `duplicated()`. `duplicated()` will mark duplicate records as TRUE and non-duplicated records as FALSE. We will add a column to our data using the trusty `$` when creating the new column. 
...
unique_results <- results %>% 
  ...
  select(-.id) 
# drop  the column containing search_terms
```
...
The creation of logical TRUE/FALSE vectors is very useful in creating conditions to filter data. In this case however, in the process note that we will lose information from the `.id` column which will become incomplete. To avoid potential confusion later on we drop the `.id` column using `select(-.id)` in the code above. If we wanted to keep the terms we would use the spread method above. 

We now have three data.frames, `results`, `wide_results`, and `unique_results`. 
...
Going forward we would use the `wide_results` and `unique_results` for further work. 

As we did earlier, use either write.csv(x, "x.csv", row.names = FALSE) or the simpler and faster `write_csv()`. R can write multiple files in a blink. This will write all three files to the rplos project folder (use `getwd()` and `setwd()` if you want to do something different, although remember to use R projects or the `here` package instead).

```{r write, eval=FALSE, tidy=TRUE}
write_csv(results, "results.csv")
write_csv(wide_results, "wide_results.csv")
...
oldham <- oldham %>% 
  separate(., author, c("author1", "author2", "author3"), sep = ",", remove = FALSE)
```

This will give us each author in a separate column. However, in many cases we will want to compile information on the publications by authors on a set of papers to do that we would use `separate_rows()`

```{r firstauthor, eval=FALSE, cache=TRUE, tidy=TRUE}
author_rows <- oldham %>% 
  separate_rows(author, sep = ",") %>% 
  mutate(author = str_trim(author, side = "both"))
```

When separating rows note that it is important to trim white space afterwards. If white space is present at the front or end of the string it will not count correctly. 

We can now easily count up the authors using count as above. This is particularly useful when dealing with larger datasets (although name disambiguation becomes an issue to avoid counting double John Smiths). For the moment,
...
sa <- plossubject(q = '"synthetic+biology"', fq = list("journal_key:PLoSONE", "doc_type:full"))$meta$numFound
```
...
plossubject(q = '"synthetic+biology"', fl = "id", fq = list("journal_key:PLoSONE", "doc_type:full"), limit = 10)
```
...
3. [RStudio Online Learning](https://www.rstudio.com/resources/training/)
